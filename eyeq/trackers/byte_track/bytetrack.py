...
from eyeq.utils.box_utils import xywh2xyxy, xyxy2xywh
...
class BYTETracker(object):
    ...
    def update_public(self, detections: sv.Detections):
        ...
        outputs = self.update(xyxy, class_ids, confidence)
        n_tracks = outputs.shape[0]

        tracklets = sv.Detections.empty()
        if n_tracks>0:
            tracklets = sv.Detections(xyxy=outputs[:, :4], class_id=outputs[:, 5].astype(int),
                                      confidence=outputs[:, 6], tracker_id=outputs[:, 4])
        return tracklets

    def update(self, xyxys, classes, confs):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
